...
import com.muralex.worldnews.presentation.ui.dialog.CountryDialogFragment
...
class HomeFragment : Fragment() {
        ...
        viewModel.viewState.observe(viewLifecycleOwner) { state ->
            when (state) {
                ...
                is ViewState.ListLoadFailure -> {
                    hideProgressBar()
                    displayErrorNotification(state.data.message.toString())
    ...
    private fun setToolbarTitle(country: String) {
    ...
    private fun hideProgressBar() {
        lifecycleScope.launch {
            delay(800) /// better visibility for the user
